...
designed to monitor and report the resource usage of a node. This local
producer collects metrics related to CPU usage, memory usage, disk I/O, and
network I/O. The collected metrics can be published to a NATS server for
real-time processing or exposed via Prometheus for integration with monitoring
dashboards.
...
- **Configurable**: Flexible configuration via command-line flags or
...
- `--node-name "node-1"`: Name of the node for identifying the source of the
  metrics.
- `--interval 10`: Interval in seconds between metric collections (default is
  10 seconds).
...
- The collector monitors CPU usage, memory usage, disk I/O, and network I/O
