...
- **NATS Integration**: Publishes collected metrics to a specified NATS
  subject, enabling seamless integration with other monitoring and
  observability tools.
...
```bash prysm local-producer kernel-metrics [flags] ```
...
kernel and can be used to identify performance bottlenecks, security issues,
...
```bash prysm local-producer kernel-metrics --nats-url "nats://localhost:4222"
